...
mystring_t *mystring_new() {
    ...
    retval = (struct mystring_t *)malloc(1 * sizeof(mystring_t));
    ...
    retval->size = 1;
    retval->data =(char *)malloc(retval->size*sizeof(char));
    
    /* Check the data attribute of our string to make sure we got memory */
    if (!retval->data) {
...
char mystring_get(mystring_t *s, size_t loc) {
    if(s == NULL) {
        fprintf(stderr, "string_get: passed a NULL string.\n");
        abort();
    }
